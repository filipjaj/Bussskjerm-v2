...
struct Config {
  ...
  char password[64];
  char endpoint[128];
  uint8_t format;       // kept for EEPROM layout compatibility
  bool configured;
  char checksum;
};
...
void handleRoot() {
    ...
    html += "var body='ssid='+encodeURIComponent(sel)+'&password='+encodeURIComponent(p)+'&endpoint='+encodeURIComponent(e);";
...
void handleSave() {
  ...
  config.format = 0;  // unused, kept for EEPROM compatibility
...
void parseBusData() {
  ...
  // Verbose format: {"departures": [{"line": "31", "destination": "Tonsenhagen", "minutes": 0}, ...]}
...
void displayBusData() {
  ...
  // Col 1: Line number   x=0,  width=20
  // Col 2: Destination   x=22, width=~70
  // Col 3: Minutes       x=98, right-aligned
  const int COL_LINE = 0;
  const int COL_DEST = 22;
  const int COL_MIN  = 98;
  ...
  for (int i = startEntry; i < endEntry; i++) {
    // Bussnummer i invertert boks
    int lineLen = strlen(departures[i].line);
    int boxWidth = max(14, lineLen * 6 + 4);
    display.fillRoundRect(COL_LINE, y - 1, boxWidth, 9, 2, SSD1306_WHITE);
    display.setCursor(COL_LINE + 2, y);
    display.setTextColor(SSD1306_BLACK);
    display.print(departures[i].line);
    display.setTextColor(SSD1306_WHITE);

    // Destinasjon
    if (departures[i].destination[0] != '\0') {
      display.setCursor(COL_DEST, y);
      char dest[14];
      sanitizeNorwegian(dest, departures[i].destination, 13);
      display.print(dest);
    }

    // Minutter (høyrejustert med "m" suffiks)
    display.setCursor(COL_MIN, y);
...
void setup() {
  ...
  Wire.begin(14, 12);  // HW-364A: SDA=GPIO14 (D5), SCL=GPIO12 (D6)
